...
**"뭐 하는 거예요?"** 면 설명:
> `~/.config/shiftee-cli/config.json`에 저장된 account_token, employee_token과 로그인 이메일을 삭제합니다. (비밀번호는 애초에 저장되지 않습니다.)
> 다시 쓰려면 `shiftee login`을 실행해서 이메일/비밀번호로 다시 로그인하면 됩니다.
